...
\section{Tuning wiht \texttt{mlrHyperopt}}

\begin{frame}[fragile]{Tuning parameters with \texttt{mlrHyperopt}}
A heuristic decides for tuing method:
...
Some jobs exceeded walltime of \SI{24}{\hour} on a Docker Cluster (Intel Xeon E5-2697v2).
\vspace{1em}
<<caretTimeOuts, echo=FALSE, fig.width=12, fig.height=8.5, results='asis'>>=
res[is.na(done), .(learner, Method)] %>% table() %>% xtable() %>% print(booktabs = TRUE, latex.environments=c("center", "footnotesize"))
@
\end{frame}

\begin{frame}{Reliability}
Some data sets caused errors in combination with certain learners.
\vspace{1em}
<<Errors, echo=FALSE, fig.width=12, fig.height=8.5, results='asis'>>=
tmp = res[!is.na(error), .(learner, Method, problem)]
tmp$problem = stri_sub(tmp$problem, 1, 3)
tmp = tmp[, {
  x = table(problem)
  x = x[x>0]
  paste0(names(x), " (", x, ")", collapse = "\\")}, by = .(learner, Method)]
reshape2::dcast(tmp, learner~Method) %>% xtable() %>% print(booktabs = TRUE, sanitize.text.function = identity, latex.environments=c("center", "footnotesize"), include.rownames = FALSE)
@
\vspace{0.5em}
\emph{Int:} Internet-Advertisements -- \emph{mon:} monks-problems-2
\end{frame}

\begin{frame}{Subset Results}
Subset results to problems that ran sucsessfully. Remove the following combinations:
\vspace{1em}
<<RemoveProblems, results='asis', echo = FALSE>>=
res[exclude == TRUE, .(learner, problem)] %>% reshape2::dcast(learner~problem) %>% xtable() %>% print(booktabs = TRUE, latex.environments=c("center", "footnotesize"), include.rownames = FALSE)
@
\vspace{1em}
Remaining unfinished runs are set to $acc = 0$.
\end{frame}
...
g + geom_line(aes(color = Method, group = paste(algorithm,search,fold)), alpha = 0.7) + facet_grid(learner~problem, scales = "free") + scale_y_log10()
@
\end{frame}
...
\vspace{1em}
The table gives the fractions of instances where $H_0: acc_A \leq acc_B$ is rejected by the paired $t$-test to level $\alpha = 0.05$. $A$ column, $B$ rows.\\
\vspace{0.5em}
\emph{i.e.:} \texttt{mlrHyperopt} is significantly better thant the default settings in \only<1>{\Sexpr{sprintf("%.0f", beat.test.mat[[1]]["mlrHyperopt","default"] * 100)}}\only<2>{\Sexpr{sprintf("%.0f", beat.test.mat[[2]]["mlrHyperopt","default"] * 100)}}\% of the cases.
\end{frame}

\begin{frame}{Ranking of the Tuning Methods}
<<rankingCombinations, echo=FALSE>>=
tmp = copy(res.def)
tmp = tmp[, {if (any(exclude == TRUE)) NULL else .SD}, by = .(problem)]
tmp[, ranking := myrank(-measure, ties.method = "first"), by = .(learner, problem, fold, budget)]
g = ggplot(tmp, aes(x = ranking, fill = Method))
g + geom_bar() + facet_grid(~budget)
@
The distribution of the rankings indicate that \texttt{caret}s grid search is indeed the best. But probably in many cases not statisitcally significant.
\end{frame}
...
tmp = tmp[, {
  ...
  mySD = copy(.SD)
  averagedSD = mySD[, list(mean.measure = mean(measure)) , by = .(learner, Method)]
  # get measures per fold for the learner Method combination with the top mean
  topSD = merge(mySD, averagedSD[which.max(mean.measure), .(learner, Method)])
  ...
  mySD[, list(p.value = t.test(x = measure, y = topmeasure, alternative = "less", paired = TRUE)$p.value, top.com = topSD$Combination[1], mean.measure = mean(measure)) , by = .(learner, Method)]
} , by = .(problem, budget)]
tmp = tmp["default" != algorithm]
tmp[, ranking := myrank(-measure, ties.method = "first"), by = .(problem, fold, budget)]
g = ggplot(tmp, aes(x = ranking, fill = Combination, color = algorithm))
g = g + geom_bar() + facet_grid(budget~.)
c7 = c('#7fc97f','#beaed4','#fdc086','#ffff99','#386cb0','#f0027f','#bf5b17')
g + scale_fill_manual(values = rep(c7, each = 3))
@
The distribution of the rankings indicate that \texttt{caret}s grid search is indeed the best. But probably in many cases not statisitcally significant.
\end{frame}
...
\begin{itemize}
  \item Parameter Tuning is only frutile on some data and for some methods
  \item \texttt{caret}s grid search has performance problems on big datasets (ksvm, nnet)
  \item \texttt{caret}s grid search submodel trick is beneficial (glmnet)
  \item \texttt{mlrHyperopt}s bayesian optimization causes considerable overhead for small datasets.
  \item benchmark indicates that random search is better than the grid search
  \item For \texttt{caret}s grid search the parameter space expands with increaseing \texttt{tuneLength}.
