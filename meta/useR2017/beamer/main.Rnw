...
Some jobs exceeded wall time of \SI{24}{\hour} on a Docker Cluster (Intel Xeon E5-2697v2).
\vspace{1em}
...
Subset results to problems that ran successfully. Remove the following combinations:
\vspace{1em}
...
<<resultsRunTime, echo=FALSE, fig.width=11, fig.height=8>>=
